...
import "./PostListHealth.css";
import { request } from "../share/request";
import { Circles } from "react-loader-spinner";
import { Button, Divider, Input, Pagination } from "antd";
import { useTranslation } from "react-i18next";
import PostCardHealth from "./PostCardHealth";
import CardHeUI from "./CardHUI";

const PostListHealth = () => {
  ...
  const getListNews = async (objFilter) => {
    ...
    const res = await request("postshealth?" + params.toString(), "get", {});
      ...
      <div className="post-list">
        {filteredNews && filteredNews.length > 0 ? (
          filteredNews.map((post, index) => (
            <div key={index}>
              {/* <PostCardHealth key={index} post={post} /> */}
              <CardHeUI key={index} post={post} />
            </div>
          ))
        ) : (
